...
# to log file
# PayUnfreezeAccount interface TestScript
def PayUnfreezeAccount():
	TIME = time.strftime('%Y/%m/%d %H:%M:%S',time.localtime(time.time()))
	client = suds.client.Client(URL)

	paramStruct = {u'PayUnfreezeAccountReq': {
			 u'MerchantId': 'CTRP',
	...
	result = client.service.PayUnfreezeAccount(**paramStruct)
	print result

	f = open(_THEPATH+'/result/'+'PayUnfreezeAccount-testReport.txt','a')
	f.writelines(TIME+'\n'+'Request:\n'+str(paramStruct)+'\n\n')
