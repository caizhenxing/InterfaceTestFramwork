...
# to log file
# QueryRechargeRecordCount interface TestScript
def QueryRechargeRecordCount():
	TIME = time.strftime('%Y/%m/%d %H:%M:%S',time.localtime(time.time()))
	client = suds.client.Client(URL)

	paramStruct = {u'QueryRechargeRecordCountReq': {
			 u'Status': u'Status',
	...
	result = client.service.QueryRechargeRecordCount(**paramStruct)
	print result

	f = open(_THEPATH+'/result/'+'QueryRechargeRecordCount-testReport.txt','a')
	f.writelines(TIME+'\n'+'Request:\n'+str(paramStruct)+'\n\n')
